...
$t->is(count($codes), 8, '"8" codes available');
$t->is($codes[0]->getCodeFormated(), 'INV.-123-Neo', 'The Code is well "INV.-123-Neo"');
$t->is($codes[1]->getCodeFormated(), '1', 'The Code is well "1"');
$t->is($codes[2]->getCodeFormated(), 'VERT. 12456', 'The Code is well "VERT. 12456"');
$t->is($codes[3]->getCodeFormated(), 'VERT./1548548 Abou', 'The Code is well "VERT./1548548 Abou"');
$t->is($codes[4]->getCodeFormated(), 'VERT./85486846', 'The Code is well "VERT./85486846"');
$t->is($codes[5]->getCodeFormated(), 'VERT./-*AFTER', 'The Code is well "VERT./-*AFTER"');
$t->is($codes[6]->getCodeFormated(), 'TEST-12345-AFTER', 'The Code is well "TEST-12345-AFTER"');
$t->is($codes[7]->getCodeFormated(), 'TEST-54321-AFTER', 'The Code is well "TEST-54321-AFTER"');

$codesPrefix = Doctrine::getTable('Codes')->getDistinctPrefixSep();

$t->is($codesPrefix->count(), 4, '"4" different prefix separators exists');
...
$t->is($codesSuffix->count(), 5, '"5" different suffix separators exists');
