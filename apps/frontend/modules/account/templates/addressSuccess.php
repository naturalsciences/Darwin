...
<form class="edition qtiped_form" action="<?php echo url_for('account/address?ref_id='.$sf_request->getParameter('ref_id') . ($form->getObject()->isNew() ? '': '&id='.$form->getObject()->getId() ) );?>" method="post" id="address_form">
  ...
  <tfoot>
    <tr>
      <td colspan="2">
	   ...
	   <a class="widget_row_delete" href="<?php echo url_for('catalogue/deleteRelated?table=users_addresses&id='.$form->getObject()->getId());?>" title="<?php echo __('Are you sure ?') ?>">
	    <?php echo __('Delete');?>
	   </a>
        <?php endif;?> 
