<table class="property_values comments"  id="spec_ident_comments">
  ...
  <?php foreach($form['Comments'] as $form_value):?>
     <?php include_partial('specimen/spec_comments', array('form' => $form_value));?>
  <?php endforeach;?>
     ...
     <tr>
       <td colspan="3">
         <div class="add_collectors">
           <a href="<?php echo url_for('parts/addComments'.($form->getObject()->isNew() ? '': '?id='.$form->getObject()->getId()) );?>/num/" id="add_comment"><?php echo __('Add Comm.');?></a>
         </div>
       </td>
     </tr>
   </tfoot>
