...
GRANT SELECT ON specimens_flat TO d2viewer;
    ...
    (SELECT spec.id,  spec.category,  spec.host_relationship,  spec.acquisition_category,  spec.acquisition_date_mask,
            ...
            ( select array_agg(DISTINCT people_ref) from catalogue_people p INNER JOIN identifications i ON p.record_id = i.id AND p.referenced_relation = 'identifications' where i.referenced_relation='specimens' and p.people_type='identifier' and i.record_id = spec.id ),
            ( select array_agg(DISTINCT people_ref) from catalogue_people where referenced_relation='specimens' and people_type='collector' and record_id = spec.id),
            ( select array_agg(DISTINCT people_ref) from catalogue_people where referenced_relation='specimens' and people_type='donator' and record_id = spec.id),
