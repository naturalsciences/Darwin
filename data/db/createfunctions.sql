...
BEGIN
	IF table_name = 'chronostratigraphy' THEN
		...
		response := true;
	ELSIF table_name = 'lithostratigraphy' THEN
		EXECUTE 'UPDATE lithostratigraphy ' ||
			'SET group_ref = ' || coalesce(parent_hierarchy_ref[1], 0) || ', ' ||
			'    group_indexed = ' || quote_literal(coalesce(parent_hierarchy_indexed[1], '')) || ', ' ||
			'    formation_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' = ''group'' THEN coalesce(formation_ref,0) ' ||
			'                   ELSE ' || coalesce(parent_hierarchy_ref[2], 0) || ' END, ' ||
			'    formation_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' = ''group'' THEN coalesce(formation_indexed,'''') ' ||
			'                       ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[2], '')) || ' END, ' ||
			'    member_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''group'', ''formation'') THEN coalesce(member_ref,0) ' ||
			'                       ELSE ' || coalesce(parent_hierarchy_ref[3], 0) || ' END, ' ||
			'    member_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''group'', ''formation'') THEN coalesce(member_indexed,'''') ' ||
			'                           ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[3], '')) || ' END, ' ||
			'    layer_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''group'', ''formation'', ''member'') THEN coalesce(layer_ref,0) ' ||
			'                      ELSE ' || coalesce(parent_hierarchy_ref[4], 0) || ' END, ' ||
			'    layer_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''group'', ''formation'', ''member'') THEN coalesce(layer_indexed,'''') ' ||
			'                      ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[4], '')) || ' END, ' ||
			'    sub_level_1_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''group'', ''formation'', ''member'', ''layer'') THEN coalesce(sub_level_1_ref,0) ' ||
			'                           ELSE ' || coalesce(parent_hierarchy_ref[5], 0) || ' END, ' ||
			'    sub_level_1_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''group'', ''formation'', ''member'', ''layer'') THEN coalesce(sub_level_1_indexed,'''') ' ||
			'                               ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[5], '')) || ' END, ' ||
			'    sub_level_2_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''group'', ''formation'', ''member'', ''layer'', ''sub_level_1'') THEN coalesce(sub_level_2_ref,0) ' ||
			'                           ELSE ' || coalesce(parent_hierarchy_ref[6], 0) || ' END, ' ||
			'    sub_level_2_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''group'', ''formation'', ''member'', ''layer'', ''sub_level_1'') THEN coalesce(sub_level_2_indexed,'''') ' ||
			'                               ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[6], '')) || ' END ' ||
			'WHERE id <> ' || parent_id || ' ' ||
			'  AND ' || quote_ident(parent_old_level_sys_name::varchar || '_ref') || ' = ' || parent_id;
		response := true;
	ELSIF table_name = 'mineralogy' THEN
		EXECUTE 'UPDATE mineralogy ' ||
			...
			'    unit_division_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' = ''unit_class'' THEN coalesce(unit_division_ref,0) ' ||
			'                   ELSE ' || coalesce(parent_hierarchy_ref[2], 0) || ' END, ' ||
			'    unit_division_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' = ''unit_class'' THEN coalesce(unit_division_indexed,'''') ' ||
			'                       ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[2], '')) || ' END, ' ||
			'    unit_family_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''unit_class'', ''unit_division'') THEN coalesce(unit_family_ref,0) ' ||
			'                       ELSE ' || coalesce(parent_hierarchy_ref[3], 0) || ' END, ' ||
			'    unit_family_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''unit_class'', ''unit_division'') THEN coalesce(unit_family_indexed,'''') ' ||
			'                           ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[3], '')) || ' END, ' ||
			'    unit_group_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''unit_class'', ''unit_division'', ''unit_family'') THEN coalesce(unit_group_ref,0) ' ||
			'                      ELSE ' || coalesce(parent_hierarchy_ref[4], 0) || ' END, ' ||
			'    unit_group_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''unit_class'', ''unit_division'', ''unit_family'') THEN coalesce(unit_group_indexed,'''') ' ||
			'                      ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[4], '')) || ' END, ' ||
			'    unit_variety_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''unit_class'', ''unit_division'', ''unit_family'', ''unit_group'') THEN coalesce(unit_variety_ref,0) ' ||
			'                           ELSE ' || coalesce(parent_hierarchy_ref[5], 0) || ' END, ' ||
			'    unit_variety_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''unit_class'', ''unit_division'', ''unit_family'', ''unit_group'') THEN coalesce(unit_variety_indexed,'''') ' ||
			'                               ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[5], '')) || ' END ' ||
			'WHERE id <> ' || parent_id || ' ' ||
			'  AND ' || quote_ident(parent_old_level_sys_name::varchar || '_ref') || ' = ' || parent_id;
		response := true;
	ELSIF table_name = 'taxa' THEN
		EXECUTE 'UPDATE taxa ' ||
			...
			'    kingdom_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' = ''domain'' THEN coalesce(kingdom_ref, 0) ' ||
			'			ELSE ' || coalesce(parent_hierarchy_ref[2], 0) || ' END, ' ||
			'    kingdom_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' = ''domain'' THEN coalesce(kingdom_indexed, '''') ' ||
			'			    ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[2], '')) || ' END, ' ||
			'    super_phylum_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'') THEN coalesce(super_phylum_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[3], 0) || ' END, ' ||
			'    super_phylum_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'') THEN coalesce(super_phylum_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[3], '')) || ' END, ' ||
			'    phylum_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'') THEN coalesce(phylum_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[4], 0) || ' END, ' ||
			'    phylum_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'') THEN coalesce(phylum_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[4], '')) || ' END, ' ||
			'    sub_phylum_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'') THEN coalesce(sub_phylum_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[5], 0) || ' END, ' ||
			'    sub_phylum_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'') THEN coalesce(sub_phylum_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[5], '')) || ' END, ' ||
			'    infra_phylum_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'') THEN coalesce(infra_phylum_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[6], 0) || ' END, ' ||
			'    infra_phylum_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'') THEN coalesce(infra_phylum_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[6], '')) || ' END, ' ||
			'    super_cohort_botany_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'') THEN coalesce(super_cohort_botany_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[7], 0) || ' END, ' ||
			'    super_cohort_botany_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'') THEN coalesce(super_cohort_botany_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[7], '')) || ' END, ' ||
			'    cohort_botany_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_cohort_botany'') THEN coalesce(cohort_botany_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[8], 0) || ' END, ' ||
			'    cohort_botany_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_cohort_botany'') THEN coalesce(cohort_botany_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[8], '')) || ' END, ' ||
			'    sub_cohort_botany_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_cohort_botany'', ''cohort_botany'') THEN coalesce(sub_cohort_botany_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[9], 0) || ' END, ' ||
			'    sub_cohort_botany_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_cohort_botany'', ''cohort_botany'') THEN coalesce(sub_cohort_botany_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[9], '')) || ' END, ' ||
			'    infra_cohort_botany_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'') THEN coalesce(infra_cohort_botany_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[10], 0) || ' END, ' ||
			'    infra_cohort_botany_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'') THEN coalesce(infra_cohort_botany_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[10], '')) || ' END, ' ||
			'    super_class_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'') THEN coalesce(super_class_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[11], 0) || ' END, ' ||
			'    super_class_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'') THEN coalesce(super_class_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[11], '')) || ' END, ' ||
			'    class_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_class'') THEN coalesce(class_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[12], 0) || ' END, ' ||
			'    class_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_class'') THEN coalesce(class_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[12], '')) || ' END, ' ||
			'    sub_class_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_class'', ''class'') THEN coalesce(sub_class_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[13], 0) || ' END, ' ||
			'    sub_class_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_class'', ''class'') THEN coalesce(sub_class_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[13], '')) || ' END, ' ||
			'    infra_class_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_class'', ''class'', ''sub_class'') THEN coalesce(infra_class_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[14], 0) || ' END, ' ||
			'    infra_class_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_class'', ''class'', ''sub_class'') THEN coalesce(infra_class_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[14], '')) || ' END, ' ||
			'    super_division_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'') THEN coalesce(super_division_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[15], 0) || ' END, ' ||
			'    super_division_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'') THEN coalesce(super_division_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[15], '')) || ' END, ' ||
			'    division_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'') THEN coalesce(division_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[16], 0) || ' END, ' ||
			'    division_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'') THEN coalesce(division_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[16], '')) || ' END, ' ||
			'    sub_division_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'') THEN coalesce(sub_division_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[17], 0) || ' END, ' ||
			'    sub_division_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'') THEN coalesce(sub_division_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[17], '')) || ' END, ' ||
			'    infra_division_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'') THEN coalesce(infra_division_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[18], 0) || ' END, ' ||
			'    infra_division_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'') THEN coalesce(infra_division_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[18], '')) || ' END, ' ||
			'    super_legion_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'') THEN coalesce(super_legion_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[19], 0) || ' END, ' ||
			'    super_legion_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'') THEN coalesce(super_legion_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[19], '')) || ' END, ' ||
			'    legion_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'') THEN coalesce(legion_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[20], 0) || ' END, ' ||
			'    legion_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'') THEN coalesce(legion_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[20], '')) || ' END, ' ||
			'    sub_legion_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'') THEN coalesce(sub_legion_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[21], 0) || ' END, ' ||
			'    sub_legion_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'') THEN coalesce(sub_legion_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[21], '')) || ' END, ' ||
			'    infra_legion_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'') THEN coalesce(infra_legion_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[22], 0) || ' END, ' ||
			'    infra_legion_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'') THEN coalesce(infra_legion_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[22], '')) || ' END, ' ||
			'    super_cohort_zoology_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'') THEN coalesce(super_cohort_zoology_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[23], 0) || ' END, ' ||
			'    super_cohort_zoology_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'') THEN coalesce(super_cohort_zoology_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[23], '')) || ' END, ' ||
			'    cohort_zoology_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'') THEN coalesce(cohort_zoology_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[24], 0) || ' END, ' ||
			'    cohort_zoology_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'') THEN coalesce(cohort_zoology_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[24], '')) || ' END, ' ||
			'    sub_cohort_zoology_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'') THEN coalesce(sub_cohort_zoology_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[25], 0) || ' END, ' ||
			'    sub_cohort_zoology_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'') THEN coalesce(sub_cohort_zoology_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[25], '')) || ' END, ' ||
			'    infra_cohort_zoology_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'') THEN coalesce(infra_cohort_zoology_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[26], 0) || ' END, ' ||
			'    infra_cohort_zoology_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'') THEN coalesce(infra_cohort_zoology_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[26], '')) || ' END, ' ||
			'    super_order_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'') THEN coalesce(super_order_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[27], 0) || ' END, ' ||
			'    super_order_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'') THEN coalesce(super_order_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[27], '')) || ' END, ' ||
			'    order_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'') THEN coalesce(order_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[28], 0) || ' END, ' ||
			'    order_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'') THEN coalesce(order_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[28], '')) || ' END, ' ||
			'    sub_order_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'') THEN coalesce(sub_order_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[29], 0) || ' END, ' ||
			'    sub_order_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'') THEN coalesce(sub_order_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[29], '')) || ' END, ' ||
			'    infra_order_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'') THEN coalesce(infra_order_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[30], 0) || ' END, ' ||
			'    infra_order_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'') THEN coalesce(infra_order_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[30], '')) || ' END, ' ||
			'    section_zoology_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'') THEN coalesce(section_zoology_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[31], 0) || ' END, ' ||
			'    section_zoology_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'') THEN coalesce(section_zoology_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[31], '')) || ' END, ' ||
			'    sub_section_zoology_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'') THEN coalesce(sub_section_zoology_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[32], 0) || ' END, ' ||
			'    sub_section_zoology_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'') THEN coalesce(sub_section_zoology_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[32], '')) || ' END, ' ||
			'    super_family_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'') THEN coalesce(super_family_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[33], 0) || ' END, ' ||
			'    super_family_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'') THEN coalesce(super_family_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[33], '')) || ' END, ' ||
			'    family_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'') THEN coalesce(family_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[34], 0) || ' END, ' ||
			'    family_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'') THEN coalesce(family_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[34], '')) || ' END, ' ||
			'    sub_family_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'') THEN coalesce(sub_family_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[35], 0) || ' END, ' ||
			'    sub_family_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'') THEN coalesce(sub_family_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[35], '')) || ' END, ' ||
			'    infra_family_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'') THEN coalesce(infra_family_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[36], 0) || ' END, ' ||
			'    infra_family_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'') THEN coalesce(infra_family_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[36], '')) || ' END, ' ||
			'    super_tribe_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'') THEN coalesce(super_tribe_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[37], 0) || ' END, ' ||
			'    super_tribe_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'') THEN coalesce(super_tribe_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[37], '')) || ' END, ' ||
			'    tribe_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'') THEN coalesce(tribe_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[38], 0) || ' END, ' ||
			'    tribe_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'') THEN coalesce(tribe_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[38], '')) || ' END, ' ||
			'    sub_tribe_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'') THEN coalesce(sub_tribe_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[39], 0) || ' END, ' ||
			'    sub_tribe_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'') THEN coalesce(sub_tribe_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[39], '')) || ' END, ' ||
			'    infra_tribe_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'') THEN coalesce(infra_tribe_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[40], 0) || ' END, ' ||
			'    infra_tribe_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'') THEN coalesce(infra_tribe_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[40], '')) || ' END, ' ||
			'    genus_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'') THEN coalesce(genus_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[41], 0) || ' END, ' ||
			'    genus_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'') THEN coalesce(genus_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[41], '')) || ' END, ' ||
			'    sub_genus_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'') THEN coalesce(sub_genus_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[42], 0) || ' END, ' ||
			'    sub_genus_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'') THEN coalesce(sub_genus_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[42], '')) || ' END, ' ||
			'    section_botany_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'') THEN coalesce(section_botany_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[43], 0) || ' END, ' ||
			'    section_botany_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'') THEN coalesce(section_botany_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[43], '')) || ' END, ' ||
			'    sub_section_botany_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'') THEN coalesce(sub_section_botany_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[44], 0) || ' END, ' ||
			'    sub_section_botany_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'') THEN coalesce(sub_section_botany_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[44], '')) || ' END, ' ||
			'    serie_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'') THEN coalesce(serie_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[45], 0) || ' END, ' ||
			'    serie_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'') THEN coalesce(serie_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[45], '')) || ' END, ' ||
			'    sub_serie_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'') THEN coalesce(sub_serie_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[46], 0) || ' END, ' ||
			'    sub_serie_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'') THEN coalesce(sub_serie_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[46], '')) || ' END, ' ||
			'    super_species_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'') THEN coalesce(super_species_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[47], 0) || ' END, ' ||
			'    super_species_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'') THEN coalesce(super_species_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[47], '')) || ' END, ' ||
			'    species_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'', ''super_species'') THEN coalesce(species_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[48], 0) || ' END, ' ||
			'    species_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'', ''super_species'') THEN coalesce(species_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[48], '')) || ' END, ' ||
			'    sub_species_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'', ''super_species'', ''species'') THEN coalesce(sub_species_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[49], 0) || ' END, ' ||
			'    sub_species_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'', ''super_species'', ''species'') THEN coalesce(sub_species_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[49], '')) || ' END, ' ||
			'    variety_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'', ''super_species'', ''species'', ''sub_species'') THEN coalesce(variety_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[50], 0) || ' END, ' ||
			'    variety_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'', ''super_species'', ''species'', ''sub_species'') THEN coalesce(variety_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[50], '')) || ' END, ' ||
			'    sub_variety_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'', ''super_species'', ''species'', ''sub_species'', ''variety'') THEN coalesce(sub_variety_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[51], 0) || ' END, ' ||
			'    sub_variety_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'', ''super_species'', ''species'', ''sub_species'', ''variety'') THEN coalesce(sub_variety_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[51], '')) || ' END, ' ||
			'    form_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'', ''super_species'', ''species'', ''sub_species'', ''variety'', ''sub_variety'') THEN coalesce(form_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[52], 0) || ' END, ' ||
			'    form_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'', ''super_species'', ''species'', ''sub_species'', ''variety'', ''sub_variety'') THEN coalesce(form_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[52], '')) || ' END, ' ||
			'    sub_form_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'', ''super_species'', ''species'', ''sub_species'', ''variety'', ''sub_variety'', ''form'') THEN coalesce(sub_form_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[53], 0) || ' END, ' ||
			'    sub_form_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'', ''super_species'', ''species'', ''sub_species'', ''variety'', ''sub_variety'', ''form'') THEN coalesce(sub_form_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[53], '')) || ' END, ' ||
			'    abberans_ref = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'', ''super_species'', ''species'', ''sub_species'', ''variety'', ''sub_variety'', ''form'', ''sub_form'') THEN coalesce(abberans_ref, 0) ' ||
			'			     ELSE ' || coalesce(parent_hierarchy_ref[54], 0) || ' END, ' ||
			'    abberans_indexed = CASE WHEN ' || quote_literal(parent_new_level_sys_name) || ' IN (''domain'', ''kingdom'', ''super_phylum'', ''phylum'', ''sub_phylum'', ''infra_phylum'', ''super_class'', ''class'', ''sub_class'', ''infra_class'', ''super_cohort_botany'', ''cohort_botany'', ''sub_cohort_botany'', ''infra_cohort_botany'', ''super_division'', ''division'', ''sub_division'', ''infra_division'', ''super_legion'', ''legion'', ''sub_legion'', ''infra_legion'', ''super_cohort_zoology'', ''cohort_zoology'', ''sub_cohort_zoology'', ''infra_cohort_zoology'', ''super_order'', ''order'', ''sub_order'', ''infra_order'', ''section_zoology'', ''infra_section_zoology'', ''super_family'', ''family'', ''sub_family'', ''infra_family'', ''super_tribe'', ''tribe'', ''sub_tribe'', ''infra_tribe'', ''genus'', ''sub_genus'', ''section_botany'', ''sub_section_botany'', ''serie'', ''sub_serie'', ''super_species'', ''species'', ''sub_species'', ''variety'', ''sub_variety'', ''form'', ''sub_form'') THEN coalesce(abberans_indexed, '''') ' ||
			'				 ELSE ' || quote_literal(coalesce(parent_hierarchy_indexed[54], '')) || ' END ' ||
			'WHERE id <> ' || parent_id || ' ' ||
			'  AND ' || quote_ident(parent_old_level_sys_name::varchar || '_ref') || ' = ' || parent_id;
		response := true;
	ELSIF table_name = 'lithology' THEN
