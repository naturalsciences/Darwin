...
CREATE OR REPLACE FUNCTION fct_cpy_tofulltext()
  RETURNS trigger AS
$BODY$
BEGIN
  IF TG_OP = 'INSERT' THEN
    ...
    ELSEIF TG_TABLE_NAME = 'comments' THEN
    ...
    ELSEIF TG_TABLE_NAME = 'multimedia' THEN
      NEW.search_ts := to_tsvector('simple', coalesce(NEW.title,'') ||' '|| coalesce(NEW.description,''));
