...
INSERT INTO identifications (id, referenced_relation, record_id, notion_concerned, value_defined) 
  VALUES (102, 'specimen_individuals', 1000, 'type', 'Jé #spéè!');
SELECT lives_ok('insert into catalogue_people(referenced_relation,record_id, people_type,people_sub_type, order_by,people_ref) 
...
SELECT is(1 , (SELECT count(*)::int FROM specimen_individuals WHERE ind_ident_ids @> ARRAY[1]),'check if ident in indiv');
INSERT INTO identifications (id, referenced_relation, record_id, notion_concerned, value_defined) 
  VALUES (103, 'specimen_individuals', 1000, 'sex', 'Jé #spéè!');
SELECT lives_ok('insert into catalogue_people(referenced_relation,record_id, people_type,people_sub_type, order_by,people_ref) 
...
SELECT is(1 , (SELECT count(*)::int FROM specimen_individuals WHERE ind_ident_ids = ARRAY[1,2]),'check if ident in indiv');
...
SELECT is(1 , (SELECT count(*)::int FROM specimen_individuals WHERE ind_ident_ids = ARRAY[1,2,5]),'ind ident field is updated');
