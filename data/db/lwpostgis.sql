...
CREATE OR REPLACE FUNCTION estimated_extent(text,text,text) RETURNS box2d AS
	'/usr/lib/postgresql/8.3/lib/liblwgeom', 'LWGEOM_estimated_extent'
...
CREATE OR REPLACE FUNCTION CheckAuthTrigger()
	RETURNS trigger AS 
	'/usr/lib/postgresql/8.3/lib/liblwgeom', 'check_authorization'
	LANGUAGE C;

CREATE OR REPLACE FUNCTION GetTransactionID()
	RETURNS xid AS 
	'/usr/lib/postgresql/8.3/lib/liblwgeom', 'getTransactionID'
	LANGUAGE C;
...
--    RETURNS bytea
--    AS '/usr/lib/postgresql/8.3/lib/liblwgeom','LWGEOM_AsShape'
--    LANGUAGE 'C' IMMUTABLE STRICT; -- WITH (isstrict,iscachable);
