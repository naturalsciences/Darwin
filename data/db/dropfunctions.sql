...
DROP FUNCTION IF EXISTS fct_cpy_update_children_when_parent_updated (table_name varchar, parent_id integer, parent_old_level template_classifications.level_ref%TYPE, parent_new_level template_classifications.level_ref%TYPE, parent_hierarchy_ref integer[], parent_hierarchy_indexed tsvector[]) CASCADE;
...
DROP FUNCTION IF EXISTS fct_chk_ReferencedRecord(table_name varchar,record_id integer) CASCADE;
DROP FUNCTION IF EXISTS fct_chk_AreAuthors() CASCADE;
DROP FUNCTION IF EXISTS fct_chk_Is_FirstLevel(table_name people_aliases.table_name%TYPE, record_id  people_aliases.record_id%TYPE) CASCADE;
